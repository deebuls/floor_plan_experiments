...
# ToDO change
# Robile conifguration is fixed to 4 wheel need to make it a parameter
...
def launch_spawn_entity(context, *args, **kwargs):
    ## getting namespace
    namespace = LaunchConfiguration('namespace')
    namespace_str = LaunchConfiguration('namespace').perform(context)
    print("==============namespace str", namespace_str)
    robot_name = LaunchConfiguration('robot_name')
    pose = {'x_pose': LaunchConfiguration('x_pose'),
    ...
    start_gazebo_spawner_cmd = Node(
        ...
        arguments=[
            '-entity', robot_name,
            #'-file', robot_sdf,  Do we need a sdf for robile
            '-topic', '/'+namespace_str+'/robot_description', #the topic is different for each robot
            #'-urdf',  ParameterValue(robot_description_content, value_type=str),
            '-robot_namespace', namespace,
            #'-gazebo_namespace', namespace,
            '-x', pose['x_pose'], '-y', pose['y_pose'], '-z', pose['z_pose'],
            '-R', pose['roll'], '-P', pose['pitch'], '-Y', pose['yaw']])
    return [start_gazebo_spawner_cmd]
...
def generate_launch_description():
    ...
    pose = {'x_pose': LaunchConfiguration('x_pose', default='-2.00'),
    ...
    declare_world_cmd = DeclareLaunchArgument(
        ...
        default_value=os.path.join(bringup_dir, 'worlds', 'hospital.world'),
    ...
    ld.add_action(OpaqueFunction(function = launch_spawn_entity))
